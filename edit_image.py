...
class CropImage:
    def __init__(self, update, img):
        ...
        cancel_button.pack()
        cancel_button.place(x=130, y=150, width=100)  # 130 150
        exit_button = tk.Button(
            top, text='Exit',
            command=lambda:[self.submit_exit(), top.destroy()]
        )
        exit_button.pack()
        exit_button.place(x=260, y=150, width=100)  # 130 150
...
class DimensionsImage:
    def __init__(self, update, img):  # image
        ...
        self.canvas.bind('<Button-3>', lambda event: [self.submit_coords(), top.destroy()])
